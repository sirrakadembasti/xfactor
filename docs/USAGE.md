...
4. Bağımlılıkları yükleyin ve backend'i başlatın:
   ```bash
   npm install
   npm run dev
   ```
   *Terminalde `Backend hazır: http://127.0.0.1:8000` mesajını görmelisiniz.*
...
1. Tarayıcıda açılan giriş ekranında varsayılan yönetici bilgileriyle giriş yapın:
   * **Kullanıcı Adı:** `admin` (veya backend `.env` dosyasındaki `ADMIN_USER`)
   * **Parola:** `.env` dosyasındaki `ADMIN_PASS`
2. Giriş başarılı olduğunda sistem bir JWT token üretir ve sizi ana çalışma ekranına yönlendirir.
...
2. Merkezi orkestratör (`workflow.js`) sırasıyla:
   * `package.json`, `tsconfig.json`, `.env` şablonlarını diske kilitler.
...
1. Üst menüden **`💻 Kod Editörü`** sekmesine geçin.
2. Sol tarafta üretilen tüm dosyaların (`src/`, `prisma/`, `package.json`, `.env` vb.) klasör ağacını görebilirsiniz.
...
2. Ekranın sağ üst köşesindeki **`📦 Projeyi (ZIP) İndir`** butonuna tıklayın.
3. Tarayıcınız `.env`, `prisma/`, `src/` ve `package.json` dahil tüm kaynak kodları eksiksiz bir ZIP arşivi olarak bilgisayarınıza indirecektir.
...
2. Bağımlılıkları yükleyin:
   ...
   npm install
   ```
3. Veritabanını ve Prisma şemasını senkronize edin:
   ```bash
   npx prisma generate
   npx prisma db push
   ```
4. Uygulamayı başlatın:
   ...
   npm run dev
   ```
5. Tarayıcınızda `http://localhost:3000` (veya Vite ise `http://localhost:5173`) adresini açarak uygulamanızı kullanın.
