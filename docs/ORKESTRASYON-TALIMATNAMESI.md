...
* **Görevleri:**
  ...
  3. Prisma/SQLite projelerinde `.env.example` dosyasında `DATABASE_URL="file:./dev.db"` tanımının yer alacağını şartnameye bağlar.
...
* **Görevleri:**
  ...
  2. Kodlama başlamadan önce `package.json`, `tsconfig.json` ve `.env.example` dosyalarının diske kilitlenmesini sağlar.
