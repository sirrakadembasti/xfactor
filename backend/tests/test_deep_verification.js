...
test("1.6 Bağımsız Görevlerin Eşzamanlı Dalgalara (Execution Waves) Ayrılması", () => {
    const dag = new TaskDAG();
    // Seviye 0: t1, t2 bağımsız
    dag.addTask({ id: 't1', title: 'Task 1', dependencies: [] });
    dag.addTask({ id: 't2', title: 'Task 2', dependencies: [] });
    // Seviye 1: t3 hem t1 hem t2'ye bağlı, t4 sadece t1'e bağlı
    dag.addTask({ id: 't3', title: 'Task 3', dependencies: ['t1', 't2'] });
    dag.addTask({ id: 't4', title: 'Task 4', dependencies: ['t1'] });
    // Seviye 2: t5 hem t3 hem t4'e bağlı
    dag.addTask({ id: 't5', title: 'Task 5', dependencies: ['t3', 't4'] });

    const waves = dag.getExecutionWaves();
    assert.strictEqual(waves.length, 3, "Tam olarak 3 yürütme dalgası oluşmalı");
    assert.deepStrictEqual(new Set(waves[0]), new Set(['t1', 't2']), "İlk dalga t1 ve t2 olmalı");
    assert.deepStrictEqual(new Set(waves[1]), new Set(['t3', 't4']), "İkinci dalga t3 ve t4 olmalı");
    assert.deepStrictEqual(waves[2], ['t5'], "Üçüncü dalga t5 olmalı");
});
...
await asyncTest("2.1 Hiyerarşik Klasör ve Protokol Dosyaları Üretimi", async () => {
    const testProjectDir = path.join(os.tmpdir(), 'test-protocol-project');
...
await asyncTest("2.2 Görev Tamamlanma Durumu ve TODO.md Ayrıştırma (Checkpoint Resume)", async () => {
    const testProjectDir = path.join(os.tmpdir(), 'test-resume-project');
    await fs.rm(testProjectDir, { recursive: true, force: true });
...
await asyncTest("4.1 Path Traversal Koruma ve Çok Dosyalı Yazma", async () => {
    const testDir = path.join(os.tmpdir(), 'test-code-gen');
