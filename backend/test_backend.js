...
runTest("0. Ortam yapılandırması repo içinde sabit kodlu gizli değer taşımamalı", () => {
    ...
    // Repo içinde .env dosyasının kalmaması beklenir; örnek dosya bulunur.
    assert.strictEqual(fs.existsSync(envPath), false, "backend/.env repo içinde kalmamalı");
    assert.strictEqual(fs.existsSync(envExamplePath), true, ".env.example dosyası bulunmalı");

    // Özellikle demonstrasyon amaçlı sabit anahtarlar kabul edilmez.
...
await runAsyncTest("2. SQLite Veritabanı Modülü & Tablo Yapısı", async () => {
    ...
    assert.ok(dbModule.saveProjectState, "saveProjectState fonksiyonu mevcut");
...
await runAsyncTest("3. Multi-LLM Sağlayıcı & Mock Fallback Mantığı", async () => {
    ...
    await assert.rejects(
        () => llmModule.generateLLMResponse(messages),
    ...
    const response = await llmModule.generateLLMResponse(messages, { allowMockFallback: true });
...
await runAsyncTest("4. Orchestrator Yardımcı Fonksiyonlar & Dizin Yapısı", async () => {
    const orchModule = await import('./orchestrator.js');
    assert.ok(orchModule.getProjectDir, "getProjectDir fonksiyonu mevcut");
    assert.ok(orchModule.getStatePath, "getStatePath fonksiyonu mevcut");
